...
Add AWS Credentials to Colab Secrets (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, S3_SUFFIX).
Run all cells to generate data and upload to S3.
Phase 2: Glue Crawler
Go to AWS Glue → Crawlers.
Create a crawler pointing to your fraud-raw-<suffix>/raw/ S3 bucket.
Set target database to fraud_db.
Run the crawler to create the raw table.
Phase 3: ETL Job
Go to AWS Glue → ETL Jobs.
Create a Spark job using glue_jobs/fraud_etl.py.
Set parameters:
--RAW_S3_PATH: s3://fraud-raw-<suffix>/raw/
--PROCESSED_S3_PATH: s3://fraud-processed-<suffix>/processed/
--DATABASE_NAME: fraud_db
Run the job (Worker type: G.1X, 2 workers).
Crucial: Re-run a crawler on the Processed bucket to update the catalog.
Phase 4: Athena Queries
Open Athena Query Editor.
Set output location to s3://fraud-processed-<suffix>/athena-results/.
Run the SQL files in athena_queries/ in order (01 → 04).
Phase 5: Dashboard
Re-open the Colab notebook and run the "Phase 5" cells.
